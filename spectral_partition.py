...
indexed_vertices_l = list(enumerate(vertices_l))
...
for indexed_vertex in sorted_indexed_vertices_l:
    v_index, v = indexed_vertex
    num_intra = 0
    num_inter = 0
    for neighbour in A[v_index].indices: # loop over the neighbours and check if it's in S
        if neighbour in S: 
            num_intra += 1
        else:
            num_inter += 1
    cut = cut - num_intra + num_inter
    vol_S = vol_S + degree(v_index)
...
print("the node ids of min set is: ", min_set)
